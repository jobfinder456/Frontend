...
export default function Pay() {
  ...
  useEffect(() => {
    ...
    const formattedPlanId = rawPlanId
      ? `plan_${rawPlanId}`
      : "No Plan Selected";
  ...
  const verifySubscription = async (subscriptionId) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACK_MAIN}/api/v1/verify-subscription`,
        {
  ...
  const handlePayment = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACK_MAIN}/api/v1/create-subscription`,
        {
          plan_id: planId,
          quantity: quantity,
        },
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-gray-600">
                      Plan Name: Growth ( 25 job credits )
                    </div>
                    <div className="mt-1 text-sm text-gray-600">
                      Plan ID: {planId}
                    </div>
              ...
              <div className="px-8 py-6 border-b border-gray-200">
                <div className="flex items-center gap-4">
                  ...
                  <input
                    ...
                    className="w-20 px-3 py-1 border border-gray-300 rounded-md disabled:opacity-80 cursor-not-allowed"
                  />
                </div>
              </div>
